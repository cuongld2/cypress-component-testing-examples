import CurrentWeather from '../../src/lib/components/CurrentWeather.svelte';
import type { CurrentWeather as CurrentWeatherType } from '../../src/lib/types/weather';
...
describe('CurrentWeather', () => {
  it('shows loading state', () => {
    cy.mount(CurrentWeather, { props: { weather: null, loading: true, error: null } });
    cy.contains('Loading current weather...').should('be.visible');
  });

  it('shows error state', () => {
    cy.mount(CurrentWeather, { props: { weather: null, loading: false, error: 'Failed to fetch' } });
    cy.contains('Failed to fetch').should('be.visible');
  });

  it('shows empty state', () => {
    cy.mount(CurrentWeather, { props: { weather: null, loading: false, error: null } });
    cy.contains('Search for a city to see weather').should('be.visible');
  });

  it('displays weather data correctly', () => {
    cy.mount(CurrentWeather, { props: { weather: mockCurrentWeather, loading: false, error: null } });
