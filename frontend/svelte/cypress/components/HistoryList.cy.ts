import HistoryList from '../../src/lib/components/HistoryList.svelte';
import type { History } from '../../src/lib/types/weather';
...
describe('HistoryList', () => {
  it('shows loading state', () => {
    cy.mount(HistoryList, { props: { history: null, loading: true, error: null } });
    cy.contains('Loading history...').should('be.visible');
  });

  it('shows error state', () => {
    cy.mount(HistoryList, { props: { history: null, loading: false, error: 'Failed to fetch' } });
    cy.contains('Failed to fetch').should('be.visible');
  });

  it('shows empty state', () => {
    cy.mount(HistoryList, { props: { history: null, loading: false, error: null } });
    cy.contains('No history data available').should('be.visible');
  });

  it('displays history data correctly', () => {
    cy.mount(HistoryList, { props: { history: mockHistory, loading: false, error: null } });
